(() => {
  ...
  const sideBtnToggle = document.getElementById('sideBtnToggle');
...
function toggleSidebar() {
  ...
  const icon = sideBtnToggle.querySelector('i');
  if (icon) {
    icon.setAttribute('data-feather', closed ? 'chevron-left' : 'chevron-right');
    feather.replace();
  }
}

if (sideBtnToggle) {
  sideBtnToggle.addEventListener('click', toggleSidebar);
}
  ...
  window.addEventListener('keydown', (ev) => {
    ...
    if (ev.key === '.' && !ev.repeat) { ev.preventDefault(); toggleSidebar(); };
    ...
    if (ev.key === 'Escape') { if (isOpen(settingsOverlay)) { ev.preventDefault(); closeSettings(); } if (isOpen(navOverlay)) { ev.preventDefault(); closeNav(); } if (isOpen(bmOverlay)) { ev.preventDefault(); closeBm(); } return; }
  }, { passive: false });
  ...
  (function () {
    const toggle = (id, openFn, closeFn) => {
    ...
    toggle('sideBtnSettings', openSettings, closeSettings);
    toggle('sideBtnBookmarks', openBm, closeBm);
    toggle('sideBtnNav', openNav, closeNav);
